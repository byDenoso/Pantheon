...
const codeFor=message=>/UNAUTHORIZED/.test(message)?401:/PRECONDITION|CONFLICT/.test(message)?409:/REQUIRED|NOT_ALLOWED|INVALID|SURFACE|NOT_FOUND/.test(message)?400:/OIDC/.test(message)?503:502;
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(operation==='receipt'){
      ...
      return send(res,200,{ok:true,id:result.id,effectKey:result.effectKey,readbackVerified:result.readbackVerified,authority:result.authority,replay:result.replay});
    }
    if(operation==='checkpoint'){
      ...
      return send(res,200,{ok:true,eventId:result.eventId,effectKey:result.effectKey,readbackVerified:result.readbackVerified,authority:result.authority,replay:result.replay});
    }
    if(operation==='action_transition'){
      const result=await bridge.transitionAction({
        accessKey,effectKey:value(url,'effect_key',500),actionId:value(url,'action_id',80),
        expectedStatus:value(url,'expected_status',40),targetStatus:value(url,'target_status',40),
        actor:value(url,'actor',180),blockerReason:value(url,'blocker_reason',2000),
      });
      return send(res,200,{ok:true,actionId:result.actionId,effectKey:result.effectKey,targetStatus:result.targetStatus,readbackVerified:result.readbackVerified,authority:result.authority,replay:result.replay});
    }
