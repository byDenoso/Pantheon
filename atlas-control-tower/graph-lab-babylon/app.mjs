...
import {BabylonGraphLab} from './scene.mjs';
...
function viewForFocus(source,id,maxVisible=120){
 ...
 for(let depth=0;depth<2&&frontier.length;depth++){
  ...
  for(const parent of frontier){
   for(const child of children.get(parent)||[]){if(ids.size>=maxVisible)break;ids.add(child);next.push(child)}
  }
  frontier=next;
 }
 let cur=byId.get(id);
 while(cur?.parentId&&ids.size<maxVisible){ids.add(cur.parentId);cur=byId.get(cur.parentId)}
 const nodes=source.nodes.filter(n=>ids.has(n.id));
 const valid=new Set(nodes.map(n=>n.id));
 return{nodes,edges:source.edges.filter(e=>valid.has(e.source)&&valid.has(e.target))};
}

const renderer=new BabylonGraphLab($('renderCanvas'),$('labels-layer'),{
...
$('max-labels').addEventListener('input',e=>{const v=Number(e.target.value);$('labels-out').textContent=String(v);renderer.setOptions({maxLabels:v})});
...
$('zoom-in').addEventListener('click',()=>renderer.zoom(.82));
