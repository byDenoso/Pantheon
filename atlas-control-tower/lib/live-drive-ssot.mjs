...
const numeric=value=>{const raw=text(value);if(!raw)return null;const n=Number(raw.replace(',','.'));return Number.isFinite(n)?n:null};
...
function workNode(row){return {id:text(row.work_id),type:'WORK',label:text(row.question)||text(row.work_id),status:text(row.status),domain:text(row.domain),summary:text(row.next_step),updatedAt:text(row.updated_at),metadata:{kind:text(row.kind),priority:text(row.priority),authority:text(row.authority),verification_status:text(row.verification_status),result_ref:text(row.result_ref)}}}
function eventNode(row){return {id:text(row.event_id),type:'EVENT',label:text(row.event_type)||text(row.event_id),status:text(row.status),domain:text(row.domain),summary:text(row.summary),updatedAt:text(row.created_at||row.updated_at),metadata:{source_role:text(row.source_role),target_role:text(row.target_role),correlation_id:text(row.correlation_id)}}}
function liveOps(snapshot){const actions=arr(snapshot.sections?.WORK).map(workNode),events=arr(snapshot.sections?.EVENTS).map(eventNode);return {...base(snapshot),counts:{blocked:actions.filter(row=>/BLOCK/.test(upper(row.status))).length,runs:null,success:actions.filter(row=>/DONE|PASS|COMPLETE/.test(upper(row.status))).length,readbackVerified:actions.filter(row=>/VERIFIED|PASS_READBACK/.test(upper(row.metadata?.verification_status))).length},actions,runs:[],events}}
