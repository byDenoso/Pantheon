...
export function discoverScienceShards({scienceIndex={},dataDir}={}){
  ...
  return [...byId.values()].map(item=>{
    ...
    if(!fs.existsSync(file))return {
      id,path:item.path,sha256:'',sourceVersion:undefined,
      declaredCount:Number.isFinite(Number(completeness.declared))?Number(completeness.declared):declaredFromCampaigns||undefined,
...
export function loadScienceShardCatalog({scienceIndex={},dataDir}={}){
  ...
  for(const descriptor of catalog){
    ...
    shards[descriptor.id]=JSON.parse(fs.readFileSync(path.join(dataDir,descriptor.path),'utf8'));
  }
