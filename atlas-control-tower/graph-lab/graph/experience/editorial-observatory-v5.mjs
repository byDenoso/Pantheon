...
function applyDesign(design,{persist=true}={}){const next=design==='performance'?'performance':'editorial';document.documentElement.dataset.atlasDesign=next;document.documentElement.dataset.atlasProduct='Editorial Observatory';document.documentElement.style.colorScheme='dark';const button=document.getElementById('atlas-design-toggle');if(button){button.textContent=DESIGN_LABEL[next];button.setAttribute('aria-label',`Visual atual: ${DESIGN_LABEL[next]}. Alternar visual.`)}if(persist)write({design:next});return next}
