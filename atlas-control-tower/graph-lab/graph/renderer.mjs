...
import {filamentControl,filamentStyle,pulsePhase,quadraticBezierPoint} from './filaments.mjs';
...
export class GraphLabRenderer{
  constructor(container,{onSelect,onOpen,onStats}={}){
    ...
    this.graph=new Graph({multi:true,type:'undirected'});this.source={nodes:[],edges:[]};this.focusId='system:NEXO';this.selectedId=null;this.hoverId=null;this.running=false;this.raf=0;this.lastStatsAt=0;this.sigma=null;this.destroyed=false;
    ...
    this.resizeObserver=new ResizeObserver(()=>{this.resizeOverlay();this.invalidate()});this.resizeObserver.observe(container);this.resizeOverlay();
    this.ready=this.initSigma();
  }
  async initSigma(){
    ...
    this.installEvents();this.sigma.refresh();this.fit();this.invalidate();
  }
  installEvents(){
  ...
  setGraph(source,{focusId=this.focusId}={}){
    ...
    if(this.selectedId&&!this.graph.hasNode(this.selectedId))this.selectedId=null;this.sigma?.refresh();if(this.sigma)this.fit();this.invalidate();this.emitStats();
  }
  setFocus(id){this.focusId=id;this.setGraph(this.source,{focusId:id})}
  zoom(factor){if(!this.sigma)return;const c=this.sigma.getCamera();const s=c.getState();c.animate({...s,ratio:clamp(s.ratio/factor,.05,20)},{duration:180})}
  fit(){this.sigma?.getCamera().animatedReset({duration:260})}
  reset(){this.fit()}
  toggleFlat(){return true}
  centerSelected(){if(!this.sigma||!this.selectedId||!this.graph.hasNode(this.selectedId))return;const p=this.sigma.getNodeDisplayData(this.selectedId);if(!p)return;this.sigma.getCamera().animate({x:p.x,y:p.y,ratio:Math.min(this.sigma.getCamera().getState().ratio,.7)},{duration:260})}
  start(){this.running=true;this.invalidate()}
  stop(){this.running=false;if(this.raf)cancelAnimationFrame(this.raf);this.raf=0}
  destroy(){this.destroyed=true;this.stop();this.resizeObserver?.disconnect();this.sigma?.kill();this.container.replaceChildren()}
  ...
  viewportPoint(id){if(!this.sigma)return null;const d=this.sigma.getNodeDisplayData(id);return d?this.sigma.graphToViewport({x:d.x,y:d.y}):null}
  ...
  drawFilaments(c,now,p){for(const [i,e] of this.source.edges.entries()){const a=this.viewportPoint(e.source),b=this.viewportPoint(e.target);if(!a||!b)continue;const s=filamentStyle(e,p),cp=filamentControl(a,b,this.options.filamentCurve,e.source<e.target?1:-1);c.strokeStyle=s.stroke;c.lineWidth=s.width;c.beginPath();c.moveTo(a.x,a.y);c.quadraticCurveTo(cp.cx,cp.cy,b.x,b.y);c.stroke();const active=this.hoverId&&[e.source,e.target].includes(this.hoverId)||this.selectedId&&[e.source,e.target].includes(this.selectedId);if(!active||this.options.pulseSpeed<=0)continue;const phase=pulsePhase(e.id||String(i),now,.22*this.options.pulseSpeed);const q=quadraticBezierPoint(a,cp,b,phase.t);const glow=c.createRadialGradient(q.x,q.y,0,q.x,q.y,10);glow.addColorStop(0,p.filaments.pulseHalo);glow.addColorStop(1,'rgba(0,0,0,0)');c.fillStyle=glow;c.beginPath();c.arc(q.x,q.y,10,0,Math.PI*2);c.fill();c.fillStyle=p.filaments.pulse;c.beginPath();c.arc(q.x,q.y,1.7,0,Math.PI*2);c.fill()}}
