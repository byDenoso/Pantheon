...
function setControlValue(id,value){
 ...
 if(out){
  const decimals=['perspective','max-labels','max-visible'].includes(id)?0:id==='drift'?1:2;
  out.textContent=Number(next).toFixed(decimals);
 }
}
function applyStoredControls(){
 ...
 if(prefs.preset&&PRESETS[prefs.preset]){
  ...
  if(preset)preset.value=prefs.preset;
 }
 if(prefs.dataset){
  const dataset=document.getElementById('dataset-size');
  if(dataset)dataset.value=String(prefs.dataset);
 }
 for(const [key,value] of Object.entries(prefs.options||{})){
  const id=OPTION_TO_CONTROL[key];
  if(id)setControlValue(id,value);
 }
}
function installPresetOptions(){
 ...
 if(!select)return;
 const labels={
  REFERENCE_3:'REFERENCE 3 · aprovado',
  GALACTIC_DUST:'GALACTIC DUST',
  FOCUS_REVIEW:'FOCUS REVIEW',
  MOBILE_CLEAN:'MOBILE CLEAN',
...
function currentPresetFromOptions(options){
 return Object.keys(PRESETS).find(name=>PRESETS[name]===options)||null;
}
export function patchRendererPrototype(Renderer){
 ...
 Renderer.prototype.setPreset=function(name){
  ...
  const select=document.getElementById('preset');
  if(select&&PRESETS[target])select.value=target;
...
function persistFromDom(){
 ...
 for(const id of OPTION_IDS){
  const input=document.getElementById(id);
  if(input)options[CONTROL_TO_OPTION[id]]=Number(input.value);
 }
...
function installDomPersistence(){
 ...
 for(const id of OPTION_IDS){
  const input=document.getElementById(id);
  input?.addEventListener('input',()=>mergePreferences({options:{[CONTROL_TO_OPTION[id]]:Number(input.value)}}));
 }
