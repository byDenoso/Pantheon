...
const TYPE_FOR_RECORD={
 campaign:'CAMPAIGN',paper:'RESULT',current_test:'TEST',running_test:'TEST',blocked_test:'TEST',open_test:'TEST',open_gate:'TEST',truth:'DOMAIN',state:'TEST',action:'TEST',objective:'TEST',policy:'TEST',strategy:'TEST',lesson:'TEST',rule:'TEST',person:'TEST'
};
...
export function gvizTableToRows(table){
 ...
 return table.rows.map(row=>{
  const out={};
  headers.forEach((key,i)=>{if(key)out[key]=cellValue(row.c?.[i])});
  return out;
 }).filter(row=>row.record_id||row.record_type||row.title);
}
...
export function rowsToGraph(rowsByTab){
 ...
 for(const tab of SSOT_TABS){
  ...
  for(const row of rowsByTab?.[tab]||[]){
   const recordId=text(row.record_id).trim();
   if(!recordId)continue;
   const id=`record:${tab}:${recordId}`;
   if(seen.has(id))continue;
   seen.add(id);
   const recordType=text(row.record_type).trim();
   const node={
    id,recordId,recordType,label:text(row.title||recordId),type:TYPE_FOR_RECORD[recordType]||'TEST',system,status:text(row.status||'UNKNOWN'),authority:'canonical',parentId,
    detail:text(row.detail),payloadJson:text(row.payload_json),source:text(row.source),updatedAt:text(row.updated_at),sheetTab:tab,ssotUrl:SSOT_SPREADSHEET_URL,hiddenChildren:0
   };
   nodes.push(node);
   edges.push({id:`edge:${parentId}:${id}`,source:parentId,target:id,kind:'canonical',authority:'canonical'});
  }
 }
...
function queryUrl(tab,handler){
 const params=new URLSearchParams({sheet:tab,tqx:`responseHandler:${handler}`});
 return `https://docs.google.com/spreadsheets/d/${SSOT_SPREADSHEET_ID}/gviz/tq?${params}`;
}
...
export function loadSsotTab(tab,{timeoutMs=9000,documentRef=globalThis.document,windowRef=globalThis}={}){
 ...
 return new Promise((resolve,reject)=>{
  const handler=`__nexoSsot_${tab}_${Date.now()}_${Math.random().toString(36).slice(2)}`;
  ...
  windowRef[handler]=response=>{
   if(response?.status==='error')return finish(reject,new Error(response.errors?.[0]?.detailed_message||`Drive SSOT query failed for ${tab}`));
   finish(resolve,gvizTableToRows(response?.table));
  };
  script.async=true;
  script.referrerPolicy='no-referrer-when-downgrade';
  script.onerror=()=>finish(reject,new Error(`Drive SSOT could not be loaded for ${tab}. Google authentication or browser privacy policy may have blocked the request.`));
  script.src=queryUrl(tab,handler);
  const timer=setTimeout(()=>finish(reject,new Error(`Drive SSOT timed out for ${tab}.`)),timeoutMs);
  (documentRef.head||documentRef.documentElement).appendChild(script);
 });
}

export async function loadSsotGraph(options={}){
 const entries=await Promise.all(SSOT_TABS.map(async tab=>[tab,await loadSsotTab(tab,options)]));
 return rowsToGraph(Object.fromEntries(entries));
}
