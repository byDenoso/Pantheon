...
function formatState(value){
 ...
 if(Array.isArray(value))return value.map(formatState).filter(Boolean).join('\n');
...
function contextItem(tab,row,kind){
 ...
 const sideQuest=kind==='side_quest'||kind==='human_inbox';
 return{
  kind,tab,
  recordId:clean(row.record_id||row.side_quest_id||row.effect||row.relation_id),
  title:clean(row.title||row.side_quest_id||row.effect||row.record_id),
  status:clean(row.status),
  detail:clean(row.detail||row.summary||row.blocker||row.readback||payload.detail),
  scope:clean(row.scope||row.blocks_scope||payload.scope),
  falsifier:clean(row.falsifier||payload.falsifier),
  boundary:clean(row.boundary||row.does_not_block||payload.boundary),
  authorityNote:clean(row.authority_note),
  evidenceRefs:parseRefs(row.evidence_refs??payload.evidence_refs),
  updatedAt:clean(row.updated_at||row.created_at||row.updatedAt),
  source:clean(row.source_ref||row.source),
  domain:clean(row.domain||row.lane),
  lane:clean(row.lane),
  type:clean(row.type),
  parentActionId:clean(row.parent_action_id),
  requiredResolution:clean(row.required_resolution),
  owner:clean(row.owner),
  wakeCondition:clean(row.wake_condition),
  blocksScope:clean(row.blocks_scope),
  doesNotBlock:clean(row.does_not_block),
  isHumanInbox:sideQuest&&(upper(row.type)==='HUMAN'||kind==='human_inbox')&&upper(row.status)==='WAITING'
 };
}

// Every text an item carries in which the SSOT could have named a hierarchy record,
// reduced to identifier tokens.
const bindingTokens=item=>new Set([item.recordId,item.title,item.detail,item.scope,item.boundary,item.source,item.domain,item.parentActionId,...item.evidenceRefs].join(' ').split(/[^A-Za-z0-9_-]+/).filter(Boolean));
...
function kindForRow(tab,row){
 if(tab==='SideQuests'||tab==='SIDE_QUESTS')return'side_quest';
 if(tab==='HumanInbox'||tab==='HUMAN_INBOX')return'human_inbox';
 ...
 if(type==='side_quest')return'side_quest';
 if(type==='human_inbox')return'human_inbox';
...
export function collectContext(rowsByTab={}){
 const items=[];
 for(const tab of ['Science','Engineering','NEXO','Olympus','SideQuests','SIDE_QUESTS','HumanInbox','HUMAN_INBOX'])
 ...
 for(const row of asArray(rowsByTab.SideQuests))items.push(contextItem('SideQuests',row,'side_quest'));
 for(const row of asArray(rowsByTab.HumanInbox))items.push(contextItem('HumanInbox',row,'human_inbox'));
 return dedupeContext(items);
}

const dedupeContext=items=>items.filter((item,index,all)=>all.findIndex(other=>other.tab===item.tab&&other.kind===item.kind&&other.recordId===item.recordId)===index);
...
export function extractLiveState(rowsByTab={}){
 ...
 const shape=item=>({
  recordId:item.recordId,status:item.status||'UNKNOWN',title:item.title||item.recordId,
  detail:item.requiredResolution||item.detail,scope:item.scope,effect:item.title,
  falsifier:item.falsifier,evidenceRefs:item.evidenceRefs,source:item.source,updatedAt:item.updatedAt,
  lane:item.lane,type:item.type,parentActionId:item.parentActionId,wakeCondition:item.wakeCondition,
  blocksScope:item.blocksScope,doesNotBlock:item.doesNotBlock,owner:item.owner
 });
 const sideQuests=context.filter(i=>i.kind==='side_quest'||i.kind==='human_inbox');
 return{
  loop:{
   currentState:formatState(loopRow.CURRENT_STATE??payload.CURRENT_STATE),
   nextAction:formatState(loopRow.NEXT_ACTION??payload.NEXT_ACTION),
   lastEffect:formatState(loopRow.LAST_EFFECT??payload.LAST_EFFECT),
  ...
  sideQuests:sideQuests.map(shape),
  humanInbox:sideQuests.filter(i=>i.isHumanInbox).map(shape)
 };
}

const ITEM_LABEL={mini_claim:'MINI-CLAIM',engineering_effect:'EFEITO',side_quest:'SIDE QUEST',human_inbox:'HUMAN INBOX',test:'TESTE',person:'PESSOA',person_state:'ESTADO',state:'ESTADO',objective:'OBJETIVO',policy:'POLÍTICA',rule:'REGRA',strategy:'ESTRATÉGIA',lesson:'LIÇÃO',action:'AÇÃO',truth:'VERDADE',record:'REGISTRO'};

const sideQuestDetail=item=>[
 item.requiredResolution||item.detail,
 item.parentActionId&&`Parent: ${item.parentActionId}`,
 item.wakeCondition&&`Wake: ${item.wakeCondition}`,
 item.blocksScope&&`Bloqueia: ${item.blocksScope}`,
 item.doesNotBlock&&`Não bloqueia: ${item.doesNotBlock}`
].filter(Boolean).join('\n');

const toItem=(item,extra={})=>({
 title:item.title||item.recordId,
 detail:(item.kind==='side_quest'||item.kind==='human_inbox')?sideQuestDetail(item):(item.detail||item.scope||''),
 status:item.status,
 tone:statusTone(item.status),
 meta:[ITEM_LABEL[item.kind]||upper(item.kind),item.lane||item.tab,item.recordId].filter(Boolean).join(' · '),
 ...extra
});
...
export function attachOperations(graph,rowsByTab={},meta={}){
 ...
 const humanInboxItems=unbound.filter(i=>i.isHumanInbox);
 const sideQuestItems=unbound.filter(i=>i.kind==='side_quest'||i.kind==='human_inbox');
 ...
 for(const node of nodes){
  ...
  const scopedSideQuests=items.filter(i=>i.kind==='side_quest'||i.kind==='human_inbox');
  const blockers=dedupe([
   ...
   ...items.filter(i=>isBlockedStatus(i.status)&&i.kind!=='side_quest'&&i.kind!=='human_inbox').map(i=>toItem(i)),
   ...(isRoot?unbound.filter(i=>isBlockedStatus(i.status)&&i.kind!=='side_quest'&&i.kind!=='human_inbox').map(i=>toItem(i)):[])
  ]);
  ...
  const changes=dedupe([
   ...
   ...items.filter(i=>i.updatedAt&&i.kind!=='engineering_effect'&&i.kind!=='side_quest'&&i.kind!=='human_inbox').sort(byRecency).slice(0,4).map(i=>toItem(i))
  ]);
  ...
  const integrity=[
   ...
   ...(isRoot?[
    ...
    {title:'Human Inbox',detail:humanInboxItems.length?`${humanInboxItems.length} decisão(ões) humanas aguardando`:'nenhuma decisão humana pendente',tone:humanInboxItems.length?'warn':'ok',meta:'SIDE_QUESTS'},
    {title:'Side Quests',detail:sideQuestItems.length?`${sideQuestItems.length} side quest(s); blockers não travam lane inteira`:'nenhuma side quest projetada',tone:sideQuestItems.length?'warn':'ok',meta:'fallthrough'},
  ...
  node.ops={
   ...
   rollup:{
    ...
    sideQuests:isRoot?sideQuestItems.length:scopedSideQuests.length,
    humanInbox:isRoot?humanInboxItems.length:scopedSideQuests.filter(i=>i.isHumanInbox).length,
   ...
   core:isRoot?{
    ...
    groups:[
     {id:'human-inbox',title:'Human Inbox',items:humanInboxItems.map(i=>toItem(i,{tone:'warn'}))},
     {id:'side-quests',title:'Side Quests',items:sideQuestItems.map(i=>toItem(i))},
   ...
   sections:[
    ...
    section('side-quests','Side Quests',dedupe(scopedSideQuests.map(i=>toItem(i))),'Nenhuma side quest vinculada a este escopo.'),
 ...
 graph.ops={
  ...
  counts:{
   ...
   sideQuests:sideQuestItems.length,humanInbox:humanInboxItems.length
  }
 };
 return graph;
}
