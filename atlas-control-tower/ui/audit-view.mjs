...
function itemRow(item, category) {
 const meta = [
  ...
  item.status ? (item.open === false ? 'resolvido · ' : 'em aberto · ') + esc(item.status) : '',
 ...
 return `<div class="audit-item${item.open === false ? ' is-resolved' : ''}">${head}
...
export function renderAudit(report, {onEntity}) {
 ...
 const open = Number.isFinite(report.open) ? report.open : report.total;
 host.innerHTML = `<div class="audit-head"><span>${num(report.total)} ocorrências registradas · <b>${num(open)}</b> em aberto`
  + `${open < report.total ? ` · ${num(report.total - open)} já resolvidas na migração` : ''}`
  + ` · fonte <b>${esc(report.source || 'legacy')}</b></span><small>${esc(age(report.generatedAt))}</small></div>`
  + report.categories.map(c => `<details class="audit-cat"${c.openCount ? ' open' : ''}>
   <summary><b>${esc(c.label)}</b> ${sev(c.severity)}${c.openCount ? `<span class="audit-open">${num(c.openCount)} em aberto</span>` : ''}<span class="audit-count">${num(c.count)}</span></summary>
