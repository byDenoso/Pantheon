...
export function createInspector({api,colors,state,safeUrl,onFocus,onLineage,onRelated}){
 ...
 async function inspect(id,{ui='overview'}={}){
  ...
  try{
   const d=await api.entity(id);if(mine!==seq)return;const n=d.entity;if(!n)throw Error('ENTITY_PAYLOAD_MISSING');const copy=cockpitCopy(n),audit=ui==='audit',shown=n.metadata||{},title=nodeDisplayLabel(n,38);
   $('#detail').innerHTML=`<p class="eyebrow" style="margin-top:20px">${esc(entityTypePt(n.subtype||n.type))}</p><h2 class="detail-title" title="${esc(n.label||'')}">${esc(title)}</h2>`
    ...
    +triad(copy)
    +`<div class="detail-actions"><button id="open-node">Explorar →</button><button id="lineage-node">Linhagem</button><button id="learning-node">Aprendizado relacionado</button><button id="compare-node">Comparar</button>${audit?'<button id="copy-node">Copiar ID</button>':''}</div>`
   ...
   $('#open-node').onclick=()=>onFocus(n);$('#lineage-node').onclick=()=>onLineage(n);$('#compare-node').onclick=()=>compare(n);$('#learning-node').onclick=()=>{$('#learning-section').hidden=false;renderLearningOverlay(api,id)};
