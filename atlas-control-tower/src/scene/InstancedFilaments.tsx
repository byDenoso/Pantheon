...
const EDGE_COLOR: Record<string,string> = {
  CONTEXT:'#2677a6', STRUCTURAL:'#356e9b', CO_DECLARED:'#b695ff', CROSS_DOMAIN:'#b695ff', CONTAINS:'#496e93',
...
function colorFor(edge:AtlasEdge){const kind=String(edge.type||'').toUpperCase();if(kind.includes('LEARNING'))return new Color('#a86dff');return new Color(EDGE_COLOR[kind]||'#315f84')}
...
export function InstancedFilaments({edges,nodes}:Props){
  ...
  useLayoutEffect(()=>{
    ...
    valid.forEach(edge=>{
      ...
      const bend=Math.min(.72,.13+distance*.055)*hashSign(String(edge.id||`${edge.source}:${edge.target}`));
      const control=a.clone().add(b).multiplyScalar(.5).add(perpendicular.multiplyScalar(bend));const color=colorFor(edge);
      for(let segment=0;segment<SEGMENTS;segment++){
        ...
        object.position.copy(midpoint);object.quaternion.copy(quaternion);object.scale.set(0.011,length,0.011);object.updateMatrix();matrix.copy(object.matrix);
  ...
  return <instancedMesh ref={mesh} args={[undefined,undefined,Math.max(1,edges.length*SEGMENTS)]} frustumCulled={false}>
    <cylinderGeometry args={[1,1,1,5,1,false]}/><primitive object={material} attach="material"/>
  </instancedMesh>;
}
