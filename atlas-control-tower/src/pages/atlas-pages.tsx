...
import { useLabData, useObservatoryData, useObservatoryQuestions, useScienceReadModel } from '../api/hooks';
...
import { H0StackForestPlot } from '../components/H0StackForestPlot';
import { ScientificObservationRenderer } from '../components/ScientificObservationRenderer';
...
function ObservatoryContextPanel({ state, context, navigate }: Pick<PageProps, 'state' | 'context' | 'navigate'>) {
  ...
  return <div className="observatory-context-panel">
    ...
    <div className="observatory-context-note"><span aria-hidden="true">✧</span><span>Testes, resultados e evidências ficam nas camadas textuais. Eles não poluem a navegação 2.5D.</span></div>
...
export function WeightedH0Panel({ state, onProvenance, onOpen, navigate }: { state: ReturnType<typeof useObservatoryData>; onProvenance: (title: string, items: Provenance[]) => void; onOpen: () => void; navigate?: Navigate }) {
  ...
  const stacks = state.data?.h0Stacks || [];
  const panelState: PanelState = state.state === 'LOADING' ? 'LOADING' : state.state === 'API_ERROR' ? 'API_ERROR' : data || stacks.length ? state.state === 'STALE' ? 'STALE' : 'READY' : 'EMPTY';
  return <PanelFrame id="weighted-h0-panel" icon="◒" title="H0 por stack" subtitle="Configurações e sínteses H0 realmente publicadas" freshness={state.data?.freshness} action={<button className="info-button" onClick={onOpen} aria-label="Abrir detalhes de H0">ⓘ</button>}><PanelStateView state={panelState} empty="Nenhuma estimativa H0 publicada." reason={SYNTHESIS_UNAVAILABLE_REASON} action={<SynthesisUnavailableAction navigate={navigate}/>}>{stacks.length > 0 && <H0StackForestPlot items={stacks}/>} {data && <><div className="hero-estimate"><div><span className="eyebrow">ESTIMATIVA CONSOLIDADA PUBLICADA</span><strong>H<sub>0</sub> = {formatEstimate(data.value, data.uncertainty)}</strong><span>{data.unit || 'Unidade não publicada'}</span></div><div className="estimate-meta">{data.datasetCount !== undefined && <span><b>{formatNumber(data.datasetCount, 0)}</b><small>datasets</small></span>}{data.chiSquared !== undefined && <span><b>χ² {formatNumber(data.chiSquared, 2)}</b><small>ajuste</small></span>}</div></div><ForestPlot estimate={data}/><div className="panel-footnote"><span>Intervalo 1σ: {data.interval1Sigma ? `[${formatNumber(data.interval1Sigma[0])}, ${formatNumber(data.interval1Sigma[1])}]` : 'não publicado'}</span><span>2σ: {data.interval2Sigma ? `[${formatNumber(data.interval2Sigma[0])}, ${formatNumber(data.interval2Sigma[1])}]` : 'não publicado'}</span><button className="source-link" onClick={() => onProvenance('Proveniência · H0 ponderado', data.provenance)}>ⓘ Fonte</button></div></>}</PanelStateView></PanelFrame>;
}
...
export function ObservatoryPage({ api, state, actions, context, navigate, onProvenance }: PageProps) {
...
export function LaboratoryPage({ api, state, actions, context, reducedMotion, compact, navigate }: PageProps) {
  ...
  useEffect(() => {
    ...
    const all = [...data.data.hypotheses, ...data.data.claims, ...data.data.tests, ...data.data.runs, ...data.data.results, ...data.data.evidence, ...data.data.decisions, ...data.data.knowledge, ...data.data.pipelines];
    const match = all.find(record => record.id === context.entity);
    if (match) openRecord(match);
  }, [context.entity, data.data]);
  return <div className="page-wrap laboratory-page"><div className="page-heading"><div><span className="eyebrow">NEXO ATLAS / LABORATÓRIO</span><h1>Laboratório</h1><p>Pipeline investigativo completo sobre registros publicados, sem transformar testes em nós do grafo estrutural.</p></div><span className="read-only-chip">READ-ONLY</span></div>{context.kind && <div className="lab-deep-link-banner" role="status">Filtrado a partir da busca · tipo <b>{context.kind}</b>{context.entity ? <> · <code>{context.entity}</code></> : null}</div>}<FilterBar area="lab" context={context} navigate={navigate}/><ReadOnlyNotice/><div className="lab-layout"><PanelFrame id="lab-knowledge-map" icon="✧" title="Mapa contextual" subtitle="O mapa mantém só a hierarquia estrutural" className="lab-map-panel"><CompactGraph state={state} actions={actions} reducedMotion={reducedMotion} compact={compact} context={context} navigate={navigate}/></PanelFrame><section className="lab-main"><div className="pipeline-strip" aria-label="Pipeline científico"><span>HYPOTHESIS</span><i>→</i><span>CLAIM</span><i>→</i><span>TEST</span><i>→</i><span>RUN</span><i>→</i><span>RESULT</span><i>→</i><span>EVIDENCE</span><i>→</i><span>DECISION</span><i>→</i><span>KNOWLEDGE</span></div><PanelFrame id="lab-stages" icon="◇" title="Trabalho registrado" subtitle={context.domain ? `Recorte ${context.domain}` : 'Todos os domínios'} freshness={data.freshness}><PanelStateView state={data.state} empty="Nenhum registro investigativo publicado neste domínio.">{data.data && <div className="lab-stages"><LabStage label="Hipóteses" count={stageCount(data.data.hypotheses)} records={data.data.hypotheses} onOpen={openRecord}/><LabStage label="Claims" count={stageCount(data.data.claims)} records={data.data.claims} onOpen={openRecord}/><LabStage label="Testes" count={stageCount(data.data.tests)} records={data.data.tests} onOpen={openRecord}/><LabStage label="Runs / execuções" count={stageCount(data.data.runs)} records={data.data.runs} onOpen={openRecord}/><LabStage label="Resultados" count={stageCount(data.data.results)} records={data.data.results} onOpen={openRecord}/><LabStage label="Evidências" count={stageCount(data.data.evidence)} records={data.data.evidence} onOpen={openRecord}/><LabStage label="Decisões" count={stageCount(data.data.decisions)} records={data.data.decisions} onOpen={openRecord}/><LabStage label="Conhecimento" count={stageCount(data.data.knowledge)} records={data.data.knowledge} onOpen={openRecord}/><LabStage label="Pipelines" count={stageCount(data.data.pipelines)} records={data.data.pipelines} onOpen={openRecord}/></div>}</PanelStateView></PanelFrame><PanelFrame icon="◌" title="Execuções recentes" subtitle="Somente registros publicados"><RecordList records={data.data?.runs || []} empty="Nenhuma execução recente publicada." onOpen={openRecord}/></PanelFrame></section></div></div>;
}

export function UniversePage({ api, state, context, navigate, onProvenance }: PageProps) {
  const data = useObservatoryData(api, context);
  const science = useScienceReadModel(api);
