...
export function ObservatorySpatialMap({state,actions,reducedMotion,compact,signals}:{state:AtlasUiState;actions:AtlasActions;reducedMotion:boolean;compact:boolean;signals?:ReactNode}){
  ...
  return <div className={`page-wrap observatory-spatial-route graphs-page spatial-knowledge-page ${immersive?'is-immersive':''}`}>
    <section className="graph-workspace spatial-workspace" id="map-workspace">
      ...
      <div className="spatial-top-hud">
        ...
        <div className="spatial-mode-switch" aria-label="Modo do mapa">
      ...
      <div className="graph-stage spatial-stage">
        {projection?<GraphRenderer projection={projection} learning={false} selectedId={state.selectedId} onSelect={select} onOpenNode={open}/>:<div className="graph-empty-state"><span aria-hidden="true">∅</span><p>{state.loading?'Lendo mapa de conhecimento…':'Mapa indisponível neste momento.'}</p><small>{state.error||'Nenhum recorte válido foi publicado.'}</small></div>}
        ...
        <div className="spatial-navigation-hud" aria-label="Controles de navegação">
          ...
          <button onClick={()=>setImmersive(value=>!value)} aria-pressed={immersive} title="Modo foco · F" aria-label="Modo foco">{immersive?'□':'⛶'}</button>
        </div>
        {signals&&<details className="observatory-signals-drawer"><summary>Observações e sinais</summary><div className="observatory-signals-content">{signals}</div></details>}
