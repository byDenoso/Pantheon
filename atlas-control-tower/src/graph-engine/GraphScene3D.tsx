  ...
  return <section className={`graph-3d-shell ${learning?'is-learning':''}`} data-graph-mode={learning?'learning-overlay':'structure'} data-motion={reducedMotion?'reduced':'auto-orbit'}>
    ...
    <div className="graph-3d-toolbar graph-3d-hud">
      ...
      <div className="graph-3d-controls" aria-label="Controles do grafo">
        ...
        <button className={autoOrbit?'active':''} aria-pressed={autoOrbit} disabled={reducedMotion} onClick={()=>setAutoOrbit(value=>!value)}>{autoOrbit?'Pausar movimento':'Mover grafo'}</button>
    ...
    <div className="graph-3d-nav-hint">movimento automático ligado · arraste rotaciona · scroll aproxima · clique abre subgrafo</div>
