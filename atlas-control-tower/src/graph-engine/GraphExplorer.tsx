  ...
  void loadGraphRuntime().then(async({PIXI,gsap})=>{
   ...
   const draw=()=>{
    ...
    for(const edge of projection.edges){
     ...
     structural.moveTo(x1,y1).bezierCurveTo((x1+x2)/2,y1-18,(x1+x2)/2,y2+18,x2,y2).stroke({width:edge.type==='CONTEXT'?1.2:1.8,color:0x426dff,alpha:dim?0.14:0.42});
    }
    ...
    if(learning){
     ...
     for(const edge of learningEdges){
      ...
      layer.moveTo(x1,y1).bezierCurveTo((x1+x2)/2,y1-42,(x1+x2)/2,y2+42,x2,y2).stroke({width:2.6,color:cross?0x58e7ff:0xb16cff,alpha:0.78});
     }
    ...
    for(const node of projection.nodes){
     ...
     graphics.circle(0,0,radius).fill({color:colorFor(node.type),alpha:selectedId&&selectedId!==node.id?0.52:0.95}).stroke({width:selectedId===node.id?3:1.2,color:0xd7f7ff,alpha:selectedId===node.id?0.95:0.38});
     wrap.addChild(graphics);
     const label=new PIXI.Text({text:node.label,style:{fontFamily:'Recursive,system-ui',fontSize:node.type==='ROOT'?15:12,fill:0xeaf7ff,fontWeight:'600',align:'center'}});label.anchor.set(0.5,0);label.y=radius+12;wrap.addChild(label);
    ...
    stage.scale.set(zoom);gsap.fromTo(stage,{alpha:0},{alpha:1,duration:0.34,ease:'power2.out'});
   };
