 ...
 useEffect(()=>{
  ...
  void loadGraphRuntime().then(async({PIXI,gsap})=>{
   ...
   const draw=()=>{
    ...
    for(const node of visibleNodes){const p=projectNode(node,width,height),wrap=new PIXI.Container();wrap.x=p.x;wrap.y=p.y;wrap.scale.set(p.depthScale);wrap.eventMode='static';wrap.cursor='pointer';const graphics=new PIXI.Graphics();const radius=node.type==='ROOT'?38:node.type==='DOMAIN'?30:node.type==='SUBGRAPH'?23:15;graphics.circle(0,0,radius+11).fill({color:colorFor(node.type),alpha:.065});graphics.circle(0,0,radius).fill({color:colorFor(node.type),alpha:current.selectedId&&current.selectedId!==node.id?.34:.96}).stroke({width:current.selectedId===node.id?3:1.2,color:0xd7f7ff,alpha:current.selectedId===node.id?.98:.32});wrap.addChild(graphics);if(labelIds.has(node.id)){const label=new PIXI.Text({text:node.label,style:{fontFamily:'Recursive,system-ui',fontSize:node.type==='ROOT'?15:12,fill:0xeaf7ff,fontWeight:'600',align:'center'}});label.anchor.set(.5,0);label.y=radius+12;wrap.addChild(label)}wrap.on('pointertap',()=>{if(gestureMovedRef.current){gestureMovedRef.current=false;return}callbacksRef.current.onSelect(node.id)});layers.nodes.addChild(wrap)}
