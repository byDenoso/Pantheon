...
type Props={node:Node25D;selected:boolean;highlighted:boolean;dimmed:boolean;showLabel:boolean;onSelect:(id:string)=>void};

export function GraphNode25D({node,selected,highlighted,dimmed,showLabel,onSelect}:Props){
  const style={
    '--node-x':`${node.x}%`,
    '--node-y':`${node.y}%`,
    '--node-z':`${node.z}px`,
