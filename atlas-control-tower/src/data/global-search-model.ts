...
const matches=(query:string,...values:unknown[])=>{
 const terms=clean(query).split(/\s+/).filter(Boolean);const hay=clean(values.map(v=>text(v)).join(' '));
...
function scienceResults(query:string,payload:unknown){
 ...
 for(const node of list(record(payload).nodes)){
  ...
  const id=text(node.id),domain=text(node.domain),path=domain?`/universes/science/${enc(domain)}?entity=${enc(id)}`:`/provenance?id=${enc(id)}`;
...
function olympusResults(query:string,payload:unknown){
 ...
 for(const node of list(record(payload).nodes)){
  ...
  push(out,{id,group:'Olympus',kind:text(node.type)||'ENTIDADE',title:text(node.label)||id,description:text(node.summary),status:text(node.status),path:`/universes/olympus?entity=${enc(id)}`});
 }
 return out.slice(0,8);
}
...
function learningResults(query:string,payload:unknown){
 ...
 for(const stage of list(record(payload).ladder))for(const item of list(stage.items)){
  ...
  const id=text(item.id);push(out,{id,group:'Learning',kind:text(stage.id)||'APRENDIZADO',title:text(item.relationType)||id,description:text(item.notes),status:text(item.status),path:`/learning?item=${enc(id)}`});
 }
 return out.slice(0,8);
}
...
function operationResults(query:string,ops:unknown,runs:unknown){
 ...
 for(const action of list(o.actions))if(matches(query,action.id,action.label,action.summary,action.status,action.domain)){
  const id=text(action.id);push(out,{id,group:'Operação',kind:'AÇÃO',title:text(action.label)||id,description:text(action.summary),status:text(action.status),path:`/operations?action=${enc(id)}`});
 }
 for(const event of list(o.events))if(matches(query,event.id,event.label,event.summary,event.status,event.domain)){
  const id=text(event.id);push(out,{id,group:'Operação',kind:'EVENTO',title:text(event.label)||id,description:text(event.summary),status:text(event.status),path:`/operations?event=${enc(id)}`});
 }
 for(const run of list(runs))if(matches(query,run.id,run.label,run.summary,run.status,run.domain)){
  const id=text(run.id);push(out,{id,group:'Operação',kind:'RUN',title:text(run.label)||id,description:text(run.summary),status:text(run.status),path:`/operations?run=${enc(id)}`});
 }
