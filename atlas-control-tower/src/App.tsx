...
export default function App() {
  ...
  const hydratedSpatialRoute = useRef('');
  ...
  useEffect(() => {
    if (route.area !== 'observatory' || hydratedSpatialRoute.current === route.path) return;
    const segments = route.context.graphPath || (route.context.domain ? ['science', route.context.domain.toLowerCase()] : []);
    if (!segments.length) return;
    hydratedSpatialRoute.current = route.path;
    void (async () => {
  ...
  useEffect(() => {
    if (route.area !== 'observatory' || state.loading || state.path.length < 2) return;
    const graphPath = state.path.slice(1).map(item => item.id);
    const desired = routeFor('observatory', { ...route.context, graphPath });
  ...
  return <div className="atlas-app premium-shell"><header className="atlas-topbar"><button className="menu-button" onClick={() => setSidebarOpen(value => !value)} aria-label="Abrir menu">☰</button><a className="atlas-brand" href={routeFor('observatory', route.context)} onClick={event => { event.preventDefault(); navigate(routeFor('observatory', route.context)); }}><span className="brand-orbit" aria-hidden="true">✧</span><span><b>NEXO <em>Atlas</em></b><small>MAPEANDO O UNIVERSO EM DADOS</small></span></a><nav className="top-navigation" aria-label="Navegação principal">{NAVIGATION.map(item => <a key={item.area} className={route.area === item.area ? 'active' : ''} href={routeFor(item.area, route.context)} onClick={event => { event.preventDefault(); go(item.area); }}>{item.label}</a>)}</nav><div className="top-actions"><CommandEntry value={query} onChange={setQuery} onSubmit={runSearch}/><button className="sync-button" onClick={() => void actions.sync()} disabled={state.syncing} aria-label="Sincronizar dados">↻ <span>{state.syncing ? 'Lendo…' : 'Sincronizar'}</span></button><FreshnessBadge freshness={freshness}/><button className="profile-button" onClick={() => setControlOpen(true)} aria-label="Abrir controle operacional">OPS</button></div></header><div className="atlas-body"><aside className={`atlas-sidebar ${sidebarOpen ? 'open' : ''}`}><div className="sidebar-heading"><span className="eyebrow">NAVEGAÇÃO ESPACIAL</span><button className="icon-button sidebar-close" onClick={() => setSidebarOpen(false)} aria-label="Fechar menu">×</button></div>{SYSTEMS.map(([id, icon, label]) => <button key={id} className={`graph-nav-item ${route.area === 'observatory' && state.focusId === id ? 'active' : ''}`} onClick={() => { navigate(routeFor('observatory')); void actions.focusSystem(id, label); setSidebarOpen(false); }}>{icon}<span>{label}</span></button>)}<div className="sidebar-divider"/><span className="eyebrow">DOMÍNIOS VISÍVEIS</span><div className="domain-nav">{domains.map(node => <button key={node.id} className="graph-nav-item" onClick={() => { const domain = String(node.domain || node.id.replace('domain:', '')); navigate(routeFor('observatory', { ...route.context, domain })); void actions.open(node); setSidebarOpen(false); }}>◎<span>{displayNode(node)}</span></button>)}</div><div className="sidebar-foot"><span className="tiny-orbit" aria-hidden="true">◎</span><b>Estado rastreável</b><p>Truth owners no backend.<br/>Atlas é projeção somente leitura.</p><small>React · PixiJS · Canvas 2.5D</small></div></aside><main id="atlas-main" className="atlas-main"><Suspense fallback={<div className="page-wrap panel-empty"><p>Carregando projeção…</p></div>}>{route.area === 'observatory' && <ObservatoryPage api={api} state={state} actions={actions} context={route.context} reducedMotion={reducedMotion} compact={compact} navigate={navigate} onProvenance={(title, items) => setDrawer({ title, items })}/>} {route.area === 'lab' && <LaboratoryPage api={api} state={state} actions={actions} context={route.context} reducedMotion={reducedMotion} compact={compact} navigate={navigate} onProvenance={(title, items) => setDrawer({ title, items })}/>} {route.area === 'universe' && <UniversePage api={api} state={state} actions={actions} context={route.context} reducedMotion={reducedMotion} compact={compact} navigate={navigate} onProvenance={(title, items) => setDrawer({ title, items })}/>}</Suspense>{route.area !== 'observatory' && <footer className="atlas-footer"><b>NEXO Atlas</b><span>Observatório para uma ciência mais conectada.</span><span className="footer-spacer"/><FreshnessBadge freshness={freshness}/><span>Runtime estático</span><span>v4.1</span></footer>}</main></div>{route.area !== 'observatory' && <Inspector state={state} actions={actions} onProvenance={(title, items) => setDrawer({ title, items })} onNavigate={go}/>}<ProvenanceDrawer open={Boolean(drawer)} title={drawer?.title} items={drawer?.items || []} onClose={() => setDrawer(null)} onNavigate={ref => { if (ref.url) window.open(ref.url, '_blank', 'noopener,noreferrer'); }} /><ControlPlaneDrawer open={controlOpen} onClose={() => setControlOpen(false)} /></div>;
}