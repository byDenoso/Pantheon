...
test('receipt upserts one deterministic execution_run and verifies Neon readback', async () => {
  const calls=[];
  let stored;
  ...
  const input={
    ...
    loop:'NEXO Executor · 3 Lanes',
    lane:'SCIENCE',
    status:'SUCCESS',
    actionId:null,
    summary:'scheduled bridge canary',
    checkpoint:'TERMINAL',
    resumePointer:'',
    expectedOutcome:'receipt persisted',
    observedOutcome:'receipt persisted',
  };
...
test('checkpoint upserts one runtime_event keyed by effect_key and reads it back', async () => {
  let stored;
  const fetchImpl=async (url,opts={})=>{
    if(opts.method==='POST'){
      stored=JSON.parse(opts.body);
  ...
  const result=await bridge.checkpoint({
    accessKey:'scheduled-secret',effectKey:'executor:e1',loop:'NEXO Executor · 3 Lanes',lane:'SCIENCE',
    step:'PERSIST',checkpoint:'WAITING_READBACK',resumePointer:'science:T-1',status:'CHECKPOINTED',summary:'resume later',
  });
  assert.equal(result.readbackVerified,true);
