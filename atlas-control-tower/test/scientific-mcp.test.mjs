...
test('failed Tower readback fails closed and never dispatches', async () => {
  const calls=[];
  const gateway={
    async findByFingerprint(){return null;},async persistTest(){calls.push('persist');return {accepted:true};},
...
test('MCP JSON-RPC initialize, tools/list and tools/call share the same service', async () => {
  const service={async submitUtterance(utterance,options){assert.equal(utterance,'Teste X');assert.equal(options.source,'chat');return {accepted:true,tests:[{test_id:'T-1',state:'CAPABILITY_GAP',canonical_readback:true}]};}};
