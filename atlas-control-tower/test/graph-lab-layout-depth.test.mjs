...
const scriptRef=file=>new RegExp(`src=['\"]\\./${file.replace(/[.*+?^${}()|[\]\\]/g,'\\$&')}['\"]`);
...
test('3D renderer adapter is wired before app and deploy-pinned',()=>{
 ...
 const volume=index.search(scriptRef('graph/volume-rendering.mjs'));
 const app=index.search(scriptRef('app.mjs'));
 assert.ok(volume>=0&&app>volume,'volume adapter must execute before app.mjs');
 assert.match(builder,/graph\/volume-rendering\.mjs/);
 assert.match(builder,/replaceAll\(`'\.\//,'builder must pin single-quoted entrypoints too');
