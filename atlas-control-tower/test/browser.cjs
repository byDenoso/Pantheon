...
const assertWorkspace=async(mode,panel,mapVisible)=>{const tab=page.locator(`[data-mode="${mode}"]`);assert.equal(await tab.getAttribute('aria-selected'),'true',mode+' tab not selected');assert.equal(await page.locator(panel).isVisible(),true,panel+' not visible');assert.equal(await page.locator('.universe').isVisible(),mapVisible,'.universe visibility wrong for '+mode);const top=await page.locator(panel).evaluate(el=>el.getBoundingClientRect().top);assert.ok(top<innerHeight,`${panel} did not enter the visible workspace`)};
await page.goto(process.env.ATLAS_BASE_URL||'http://127.0.0.1:3000');await page.waitForSelector('.entity-item');await page.screenshot({path:path.resolve('preview-desktop.png'),fullPage:true});await page.locator('#theme-toggle').click();assert.equal(await page.locator('html').getAttribute('data-theme'),'light');await page.screenshot({path:path.resolve('preview-light.png'),fullPage:true});await page.reload();await page.waitForSelector('.entity-item');assert.equal(await page.locator('html').getAttribute('data-theme'),'light');await page.locator('#theme-toggle').click();
...
// Learning is a real top-level workspace backed by the five explicit Neon stages.
await page.locator('[data-mode="learning"]').click();await page.waitForSelector('#learning-panel .ladder-stage');
await assertWorkspace('learning','#learning-section-panel',false);
assert.equal(await page.locator('#learning-panel .ladder-stage').count(),5);
assert.ok(await page.locator('#learning-panel .ladder-item').count()>0,'Learning stages rendered without records');
...
// Migration health is also a real workspace and lists declared categories.
await page.locator('[data-mode="audit"]').click();await page.waitForSelector('#audit-panel .audit-cat');
await assertWorkspace('audit','#audit-section',false);
...
await page.locator('[data-mode="overview"]').click();await assertWorkspace('overview','#map-workspace',true);
...
const cited=await page.evaluate(async()=>{const report=await fetch('/api/learning').then(r=>r.json());
 ...
 for(const t of tokens){const id='test:'+t;const entity=await fetch('/api/entity?id='+encodeURIComponent(id));if(!entity.ok)continue;
...
// Dados is a real workspace over the exact same recorte, not a panel hidden below the map.
await page.locator('[data-mode="explore"]').click();await page.waitForSelector('#data-panel tbody tr');
await assertWorkspace('explore','#data-section',false);
...
await page.locator('[data-mode="audit"]').click();await page.waitForSelector('#audit-panel .audit-cat');await assertWorkspace('audit','#audit-section',false);
...
await page.locator('[data-mode="learning"]').click();await page.waitForSelector('#learning-panel .ladder-stage');await assertWorkspace('learning','#learning-section-panel',false);
...
await page.locator('#home').click();await page.waitForFunction(()=>document.querySelectorAll('[data-crumb]').length===1);

// Mobile: each workspace remains functional without document overflow.
await page.setViewportSize({width:375,height:812});await page.locator('#theme-toggle').click();await page.evaluate(()=>scrollTo(0,0));await page.screenshot({path:path.resolve('preview-mobile-light.png'),fullPage:true});await page.locator('#theme-toggle').click();await page.screenshot({path:path.resolve('preview-mobile.png'),fullPage:true});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
await page.locator('[data-mode="explore"]').click();await page.waitForSelector('#data-panel tbody tr');await assertWorkspace('explore','#data-section',false);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);
await page.locator('[data-mode="learning"]').click();await page.waitForSelector('#learning-panel .ladder-stage');await assertWorkspace('learning','#learning-section-panel',false);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await page.screenshot({path:path.resolve('preview-mobile-learning.png'),fullPage:true});
await page.locator('[data-mode="audit"]').click();await page.waitForSelector('#audit-panel .audit-cat');await assertWorkspace('audit','#audit-section',false);assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await page.screenshot({path:path.resolve('preview-mobile-audit.png'),fullPage:true});
await page.locator('[data-mode="overview"]').click();await assertWorkspace('overview','#map-workspace',true);
await page.locator('#immersive').click();await page.screenshot({path:path.resolve('preview-mobile-map.png')});assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);assert.deepEqual(errors,[]);console.log('PASS desktop/mobile: Premium V2 workspaces, contract V1, datasource, naming + canonical id, Dados, Learning, Audit, graph layers, inspector, keyboard, immersion, motion/flat, no overflow or page errors');await browser.close()})().catch(e=>{console.error(e);process.exit(1)});