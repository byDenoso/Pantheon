...
test('v2 pages use one renderer gateway with Spatial Canvas 2.5D as the only active renderer',()=>{
 ...
 assert.match(gateway,/Canvas25DGraph/);
 assert.match(gateway,/deriveGraphNavigation/);
 assert.doesNotMatch(gateway,/GraphScene3D|requested===['"]webgl['"]|graph-renderer-switch|localStorage|URLSearchParams/);
});
