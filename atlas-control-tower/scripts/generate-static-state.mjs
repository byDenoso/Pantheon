...
const shards={};
for(const [name,rel] of Object.entries(science.shards||{}))shards[name]=read(path.join(dataDir,rel));
...
console.log(`NEXO_STATIC_STATE_OK fingerprint=${result.fingerprint} public=${publicManifest.fingerprint} out=${path.relative(root,outDir)}`);
