...
export class ExecutionLedger {
  ...
  ack(receiptId,effect={}){
    return this.transition(receiptId,'PROVIDER_ACK',{provider_effect_id:effect.effect_id||effect.provider_effect_id||null,source_ref:effect.source_ref||null,provider_response_classification:effect.classification||'ACK',before_revision:effect.before_revision??this.byReceipt.get(receiptId)?.before_revision??null,explanation:effect.explanation||'Provider acknowledged the action.'});
  }
  finish(receiptId,verification={}){
    const status=FINAL.has(verification.status)?verification.status:'DEGRADED';
    return this.transition(receiptId,status,{after_revision:verification.after_revision??null,readback_status:verification.readback_status||null,source_ref:verification.source_ref||this.byReceipt.get(receiptId)?.source_ref||null,explanation:verification.explanation||'Readback completed.'});
  }
