...
function mapFilaments(rows){
  return (rows||[]).filter(r=>text(r.filament_id)).map(row=>{
    const fromDomain=domainOf(row.source_domain||row.target_domain),toDomain=domainOf(row.target_domain||row.source_domain);
    return {id:text(row.filament_id),label:text(row.filament_type)||text(row.filament_id),domain:fromDomain,kind:upper(row.source_layer).includes('SEMANTIC')?'SEMANTIC':'PROCEDURAL',weight:num(row.weight),support:num(row.support_count),contradiction:num(row.contradiction_count),status:upper(row.status)==='ACTIVE'?'ESTABLISHED':upper(row.status).includes('CONTEST')?'CONTESTED':upper(row.status).includes('RETIR')||upper(row.status).includes('SUPERSE')?'RETIRED':'PROVISIONAL',evidence:text(row.evidence_refs).split(/[;,]/).map(x=>x.trim()).filter(Boolean),source_ref:text(row.source_ref)||SHEET_REF,boundary:text(row.next_discriminant)||text(row.activation_rule)||'Sem boundary explícito.',from_label:text(row.source_label||row.source_id),to_label:text(row.target_label||row.target_id),from_id:text(row.source_id)||`filament:${text(row.filament_id)}:source`,to_id:text(row.target_id)||`filament:${text(row.filament_id)}:target`,from_domain:fromDomain,to_domain:toDomain,scope:fromDomain===toDomain?'INTRA_DOMAIN':'INTER_DOMAIN'};
  });
}
...
function buildGraph({actions,capabilities,providers,envelopes,filaments,sideQuests,findings,now}){
  ...
  for(const f of filaments){
    const id=`filament:${f.id}`,fromId=f.from_id||`${id}:source`,toId=f.to_id||`${id}:target`,fromDomain=f.from_domain||f.domain,toDomain=f.to_domain||f.domain;
    add({id,type:'FILAMENT',label:f.label,domain:f.domain,state:f.status==='CONTESTED'?'DEGRADED':'LIVE',authority_class:'DERIVED',source_ref:f.source_ref,source_revision:'LEARNING',fingerprint:`FIL-${hash(f)}`,freshness:{state:'RECENT',observed_at:now,ttl_seconds:null},checked_at:now,summary:f.boundary,evidence:f.evidence});
    if(!nodes.some(n=>n.id===fromId))add({id:fromId,type:'MEMORY',label:f.from_label||fromId,domain:fromDomain,state:'LIVE',authority_class:'DERIVED',source_ref:f.source_ref,source_revision:'LEARNING',fingerprint:`MEM-${hash(fromId)}`,freshness:{state:'RECENT',observed_at:now,ttl_seconds:null},checked_at:now,summary:`Origem do aprendizado ${f.label}.`});
    if(!nodes.some(n=>n.id===toId))add({id:toId,type:'MEMORY',label:f.to_label||toId,domain:toDomain,state:'LIVE',authority_class:'DERIVED',source_ref:f.source_ref,source_revision:'LEARNING',fingerprint:`MEM-${hash(toId)}`,freshness:{state:'RECENT',observed_at:now,ttl_seconds:null},checked_at:now,summary:`Destino do aprendizado ${f.label}.`});
    edge(fromId,toId,'SUPPORTS',`Learning filament · ${f.scope||'INTRA_DOMAIN'}`,f.weight,{is_learning:true,learning_scope:f.scope||'INTRA_DOMAIN',blocked:f.status==='RETIRED'});
  }
