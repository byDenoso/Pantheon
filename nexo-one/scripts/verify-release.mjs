...
const provenance=await read('/release-provenance.json');
if(provenance?.contractVersion!=='1'||!/^[0-9a-f]{40}$/i.test(String(provenance?.gitCommit||'')))throw new Error('Release provenance missing or invalid.');
const expectedSha=String(process.env.NEXO_EXPECTED_SHA||'').trim();if(expectedSha&&provenance.gitCommit!==expectedSha)throw new Error('Release provenance SHA mismatch.');
...
const report={url:base,checkedAt:new Date().toISOString(),releaseCommit:provenance.gitCommit,publicBoundary:'pass',shell:'pass',provenance:'pass',operational:'blocked',truthGraph:'blocked'};
