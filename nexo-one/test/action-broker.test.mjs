...
const capabilityRows=[{capability_id:'CAP-GCAL-SCHEDULED-WRITE',domain:'OLYMPUS',status:'PASS'}];
const truthGraphInput={authorityRows,capabilityRows};
const intent={action_id:'A1',action_type:'calendar.create',domain:'OLYMPUS',provider:'calendar',capability_id:'CAP-GCAL-SCHEDULED-WRITE',target_ref:'primary',idempotency_key:'K1',requested_payload:{summary:'Canary'}};
...
function setup(provider={}){
  ...
  return {broker,ledger,effects:()=>effects};
}

test('plan validates authority/capability but does not dispatch',async()=>{
  const {broker,effects}=setup();const plan=await broker.planAction(intent,{env:{}});
  assert.equal(plan.status,'GATED');assert.equal(plan.confirmation_level,'CONFIRM');assert.equal(effects(),0);
});
...
test('confirmed write dispatches then readbacks before PASS',async()=>{
  const {broker,effects}=setup();const receipt=await broker.executeAction(intent,{env:{},confirmed:'CONFIRM'});
  assert.equal(effects(),1);assert.equal(receipt.status,'PASS');assert.equal(receipt.readback_status,'MATCH');assert.ok(receipt.trace.some(x=>x.stage==='PROVIDER_ACK'));assert.ok(receipt.trace.some(x=>x.stage==='READBACK'));
});
