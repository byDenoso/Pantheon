...
import {createNexoMcpWebHandler,NEXO_MCP_TOOL_NAMES,NEXO_MCP_MUTATION_TOOL_NAMES} from '../server/mcp/server.mjs';

const snapshot={
  sourceModifiedAt:'2026-09-12T12:00:00Z',generatedAt:'2026-09-12T12:01:00Z',
  sections:{WORK:[{work_id:'T-H0',thread_id:'THR::SCIENCE::ROOT',kind:'TEST',question:'H0 test',status:'DONE',primary_campaign:'CAMP-H0',source_ref:'SRC-T'}],HYPOTHESIS:[],TEST:[],RUN:[],RESULT:[],CLAIM:[],READBACK:[],EVENTS:[],KNOWLEDGE:[],DECISIONS:[],SYSTEM:[],THREADS:[]},
  projections:{Science:[
...
test('official MCP client negotiates 2026-07-28 and exposes read plus fail-closed governed capability tools',async()=>{
  ...
  try{
    ...
    for(const tool of list.tools){
      const mutation=NEXO_MCP_MUTATION_TOOL_NAMES.includes(tool.name);
      assert.equal(tool.annotations?.readOnlyHint,!mutation,tool.name);
    ...
    assert.equal(capabilityPayload.canonical.root,'TOWER_V06');
    assert.equal(capabilityPayload.mutationAvailable,false);
