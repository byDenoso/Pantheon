...
const tables={
 ...
 PROJECTS:rows(['record_type','record_id','status','title','summary','current_revision_id','domain','source_ref'],['program','PROG-1','ACTIVE','Program','Question','REV-1','SCIENCE','Drive']),
 TESTS:rows(['metatest_id','test_type','subject_type','subject_id','question','status','result','updated_at'],['MT-1','HOLDOUT','CrossDomain','CD-1','Does it survive?','COMPLETE_SUPPORT','PASS','2026-09-11T12:00:00-03:00']),
 RELATIONS:rows(['relation_id','source_domain','source_entity','relation_type','target_domain','target_entity','status'],['REL-1','SCIENCE','CAMP-1','PRIMARY_PROGRAM','SCIENCE','PROG-1','ACTIVE'])
};
...
test('Atlas SSOT snapshot exposes canonical operational and catalog sections',()=>{
 ...
 assert.deepEqual(Object.keys(snap.sections),['THREADS','WORK','EVENTS','KNOWLEDGE','DECISIONS','SYSTEM','PROJECTS','TESTS','RELATIONS']);
 assert.equal(snap.sections.WORK[0].work_id,'WORK::SCI::1');
 assert.equal(snap.sections.TESTS[0].metatest_id,'MT-1');
 assert.equal(snap.sections.RELATIONS[0].relation_id,'REL-1');
 assert.equal(snap.sections.PROJECTS[0].record_id,'PROG-1');
...
test('malformed or missing canonical tabs fail closed',()=>{
 ...
 assert.throws(()=>buildAtlasSsotSnapshot({tables:{...tables,TESTS:[['wrong_header']]},sourceFileId:'sheet-1'}),/INVALID_CANONICAL_TAB:TESTS/);
});
