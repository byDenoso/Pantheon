...
test('Olympus is CONFLICT when SSOT declares Sheets while authority matrix declares Drive',()=>{
  const out=graph({authorityRows:[authority('OLYMPUS','Olympus Drive folder/ledger/protocol for persisted data/state')],truthRows:[truth('OLYMPUS','Google Sheets:NEXO · SSOT CANONICAL / Olympus','Olympus operational truth hot state')],capabilityRows:[cap('OLYMPUS')],providers:[provider('drive'),provider('nexo')]});
...
test('Olympus canonical NEXO SSOT authority resolves to nexo instead of owner-dependent',()=>{
  const out=graph({authorityRows:[authority('OLYMPUS','NEXO · SSOT CANONICAL / Olympus')],truthRows:[truth('OLYMPUS','Google Sheets:NEXO · SSOT CANONICAL / Olympus','Olympus operational truth hot state')],capabilityRows:[cap('OLYMPUS')],providers:[provider('nexo')]});
  const finding=out.results[0];assert.equal(finding.provider.expected,'nexo');assert.equal(finding.provider.actual,'nexo');assert.equal(finding.status,'LIVE');assert.equal(finding.material,false);
});

test('AUTH_REQUIRED is DEGRADED, not a material missing-provider claim',()=>{
  const out=graph({authorityRows:[authority('SCIENCE','PEER Control Tower + SCIENCE skill')],truthRows:[truth('SCIENCE','Google Drive PEER Control Tower')],capabilityRows:[cap('SCIENCE')],providers:[provider('drive','AUTH_REQUIRED')]});
  const finding=out.results[0];
  assert.equal(finding.status,'DEGRADED');
  assert.equal(finding.material,false);
  assert.equal(finding.provider.expected_status,'AUTH_REQUIRED');
});
...
test('detects LIVE, DEGRADED, STALE_DECLARATION, MISSING_PROVIDER and BLOCKED',()=>{
  ...
  for(const [name,input,expected] of cases){const finding=graph(input).results[0];assert.equal(finding.status,expected,name);assert.match(finding.fingerprint,/^TG-/);}
});
...
test('fingerprints ignore checked_at but change when semantic state changes',()=>{
  ...
  const a=buildTruthGraph({...input,refs,now:NOW});const b=buildTruthGraph({...input,refs,now:NOW+60_000});assert.equal(a.results[0].fingerprint,b.results[0].fingerprint);
  const c=buildTruthGraph({...input,refs,now:NOW,capabilityRows:[cap('ENGINEERING','PENDING_CANARY')]});assert.notEqual(a.results[0].fingerprint,c.results[0].fingerprint);
});
