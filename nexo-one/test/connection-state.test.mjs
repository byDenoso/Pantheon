...
test('separates private session readiness from provider credential readiness',()=>{
  ...
  const result=summarizeConnectionHealth({env,providers:[provider('gmail','AUTH_REQUIRED'),provider('calendar','AUTH_REQUIRED'),provider('drive','AUTH_REQUIRED')],access:'PRIVATE'});
...
test('configured google connector remains unauthorized until a private provider succeeds at runtime',()=>{
  const env={GOOGLE_CONNECTOR:'google/account',VERCEL_OIDC_TOKEN:'oidc'};
  const result=summarizeConnectionHealth({env,providers:[provider('gmail','AUTH_REQUIRED'),provider('calendar','AUTH_REQUIRED'),provider('drive','AUTH_REQUIRED')],access:'PRIVATE'});
  assert.equal(result.connections.google.configured,true);
  assert.equal(result.connections.google.authorized,false);
  assert.equal(result.connections.google.runtimeVerified,false);
  assert.equal(result.connections.google.verificationState,'NOT_VERIFIED');
});

test('public health never claims private authorization failure from intentionally masked providers',()=>{
  const env={GOOGLE_CONNECTOR:'google/account',VERCEL_OIDC_TOKEN:'oidc'};
  const result=summarizeConnectionHealth({env,providers:[provider('gmail','AUTH_REQUIRED'),provider('calendar','AUTH_REQUIRED'),provider('drive','AUTH_REQUIRED')],access:'PUBLIC'});
  assert.equal(result.connections.google.configured,true);
  assert.equal(result.connections.google.authorized,null);
  assert.equal(result.connections.google.runtimeVerified,false);
  assert.equal(result.connections.google.verificationState,'PRIVATE_SESSION_REQUIRED');
});

test('marks google authorized only from observed private provider availability',()=>{
  const env={GOOGLE_CONNECTOR:'google/account',VERCEL_OIDC_TOKEN:'oidc'};
  const result=summarizeConnectionHealth({env,providers:[provider('gmail','AVAILABLE'),provider('calendar','AVAILABLE'),provider('drive','AVAILABLE')],access:'PRIVATE'});
  assert.equal(result.connections.google.configured,true);
