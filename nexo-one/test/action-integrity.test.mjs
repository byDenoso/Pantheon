...
const integrityTruth={authorityRows:[{domain:'NEXO',canonical_truth:'ACTION_REGISTER + NDMK/STATE_INDEX'}],capabilityRows:[{capability_id:'CAP-GDRIVE-INTERACTIVE-SHEET',domain:'NEXO',status:'PASS'}]};
...
test('Integrity persistence fails closed without current NEXO sheet capability PASS',async()=>{
  await assert.rejects(()=>persistMaterialIncident({...base,status:'CONFLICT',readback_status:'MISMATCH'},{env:{NEXO_SHEET_ID:'sheet-1'},truthGraphInput:{authorityRows:integrityTruth.authorityRows,capabilityRows:[]},tokenProvider:async()=> 'tok',requester:async()=>({values:[header]})}),e=>e.code==='CAPABILITY_BLOCKED');
});
...
test('persist dedupes existing check_id and performs no write',async()=>{
  ...
  const result=await persistMaterialIncident({...base,status:'CONFLICT',readback_status:'MISMATCH'},{env:{NEXO_SHEET_ID:'sheet-1'},truthGraphInput:integrityTruth,tokenProvider:async()=> 'tok',requester});
...
test('persist appends canonical 14-column row then proves it by readback',async()=>{
  ...
  const result=await persistMaterialIncident(receipt,{env:{NEXO_SHEET_ID:'sheet-1'},truthGraphInput:integrityTruth,tokenProvider:async()=> 'tok',requester});
