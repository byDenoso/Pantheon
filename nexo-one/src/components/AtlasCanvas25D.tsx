...
export function AtlasCanvas25D({ nodes, edges, selectedId, onSelect }: { nodes: PlacedNode3D[]; edges: GraphEdge[]; selectedId: string | null; onSelect: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      ctx.save(); ctx.translate(width / 2 + camera.panX, height / 2 + camera.panY); ctx.strokeStyle = 'rgba(132,190,224,.28)'; ctx.lineWidth = 1; ctx.beginPath(); ctx.ellipse(0, 0, Math.min(width * .37, bounds.radius * scale * camera.zoom), Math.min(height * .27, bounds.radius * scale * camera.zoom * camera.tilt), 0, 0, Math.PI * 2); ctx.stroke(); ctx.restore();
      ...
      const candidates = [...projected].filter(item => item.node.type === 'DOMAIN' || item.node.type === 'PROVIDER' || item.node.id === selectedRef.current || (camera.zoom > 1.42 && item.node.type === 'ACTION')).sort((a, b) => (PRIORITY[b.node.type] ?? 30) + b.depth * 10 - ((PRIORITY[a.node.type] ?? 30) + a.depth * 10));
      const labels: Array<{ x: number; y: number; w: number; h: number }> = [], maxLabels = width < 720 ? (camera.zoom > 1.5 ? 16 : 10) : camera.zoom > 1.3 ? 28 : 18;
      for (const item of candidates) { if (labels.length >= maxLabels) break; const text = labelFor(item.node); ctx.font = item.node.type === 'DOMAIN' ? '700 13px system-ui, sans-serif' : '600 11px system-ui, sans-serif'; const w = Math.min(230, Math.max(52, ctx.measureText(text).width + 12)), x = item.x, y = item.y + item.radius + 15, box = { x: x - w / 2, y: y - 9, w, h: 18 }; if (labels.some(other => Math.abs(other.x + other.w / 2 - x) < (other.w + w) * .43 && Math.abs(other.y + other.h / 2 - y) < 22) && item.node.id !== selectedRef.current) continue; labels.push(box); ctx.save(); ctx.globalAlpha = item.node.id === selectedRef.current ? 1 : .78 + item.depth * .22; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillStyle = '#eaf7ff'; ctx.fillText(text, x, y, w - 6); ctx.restore(); }
    ...
    const onKeyDown = (event: KeyboardEvent) => { if (event.key === 'ArrowLeft') camera.rotation -= .12; else if (event.key === 'ArrowRight') camera.rotation += .12; else if (event.key === 'ArrowUp') camera.tilt = Math.min(.92, camera.tilt + .05); else if (event.key === 'ArrowDown') camera.tilt = Math.max(.32, camera.tilt - .05); else if (event.key === '+' || event.key === '=') camera.zoom = Math.min(MAX_ZOOM, camera.zoom * 1.25); else if (event.key === '-' || event.key === '_') camera.zoom = Math.max(MIN_ZOOM, camera.zoom * .8); else if (event.key === '0' || event.key === 'Home') { camera.rotation = 0; camera.tilt = .58; camera.zoom = 1; camera.panX = 0; camera.panY = 0; } else return; event.preventDefault(); requestDraw(); };
