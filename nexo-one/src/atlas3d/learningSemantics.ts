...
function proceduralTheme(signal: string): ProceduralTheme | null {
  ...
  if (/MISSING[_ -]?EVIDENCE|PROMOT|PREREG|DECISION|UNDERIDENTIFIED|UNDERPOWERED/.test(signal)) return 'GOVERNANCE';
  if (/COVARIANCE|INDEPENDENCE|TENSION|SIGNIFICANCE|NULL|CALIBRATION|PRIOR|EVIDENCE/.test(signal)) return 'INFERENCE';
  if (/INVARIANCE|ROBUST|REPRODUC|HOLDOUT|PREPROCESS|ESTIMATOR/.test(signal)) return 'ROBUSTNESS';
  return null;
}
