...
export type GraphNodeType =
  | 'DOMAIN' | 'SUBDOMAIN'
  | 'HYPOTHESIS' | 'CAMPAIGN' | 'TEST' | 'RESULT' | 'EVIDENCE' | 'CLAIM' | 'DATASET' | 'MEMORY'
  | 'ACTION' | 'AUTOMATION' | 'RUN' | 'EFFECT' | 'ARTIFACT' | 'SIDE_QUEST'
  | 'CAPABILITY' | 'PROVIDER' | 'TOOL' | 'PROJECTION'
  | 'GOVERNANCE_RULE' | 'FILAMENT';
...
export type RelationKind =
  | 'OWNS' | 'CONTAINS' | 'BELONGS_TO' | 'PRODUCES' | 'VERIFIES' | 'TESTS'
  | 'DEPENDS_ON' | 'REQUIRES' | 'USES' | 'IMPLEMENTED_BY' | 'EXECUTES' | 'PROJECTS'
  | 'CONTRADICTS' | 'SUPPORTS' | 'COMPETES_WITH' | 'ROUTES_TO' | 'BLOCKS'
