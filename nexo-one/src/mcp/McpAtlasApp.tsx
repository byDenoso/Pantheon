...
import {
  idOf,kindLabel,matchRank,nodeMatches,systemGraphModel,
  type NodeKind,type Topology,type TopologyNode,type TopologyViewMode,
} from './topologyModel.ts';

type ViewMode=TopologyViewMode;
...
export function McpAtlasApp({themeOverride,embedded=false}:{themeOverride?:string;onThemeToggle?:()=>void;embedded?:boolean}={}){
 ...
 const [mode,setMode]=useState<ViewMode>(()=>{const m=routeParams().get('mode') as ViewMode|null;return m&&(['all','tools','capabilities','runtime','roles'] as ViewMode[]).includes(m)?m:'all';});
