...
export function McpAtlasApp({themeOverride,embedded=false}:{themeOverride?:string;onThemeToggle?:()=>void;embedded?:boolean}={}){
 ...
 <main className="system-native" data-system-tab={tab} data-system-ready={topology?'true':'false'}>
  ...
  {topology&&tab==='overview'&&<>{capCounts&&<CapabilityCountLine counts={capCounts}/>}<div className="system-stat-row">{[[topology.stats.tools,'tools',`${topology.stats.remote_tools} remotas · ${topology.stats.internal_tools} internas`],[topology.stats.capabilities,'capabilities','registradas na projeção'],[topology.stats.backends,'runtimes',topology.stats.backend_counts.unknown?`${topology.stats.backend_counts.unknown} desconhecidos`:'identificados'],[topology.stats.roles,'papéis','declarados'],[topology.links.length,'relações','publicadas']].map(([v,n,d])=><div key={String(n)}><strong>{v}</strong><span>{n}</span><small>{d}</small></div>)}</div>
