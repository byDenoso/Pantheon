...
type SystemTab='overview'|'tools'|'capabilities'|'runtimes'|'roles'|'relations'|'provenance';
const SYSTEM_TABS:Array<[SystemTab,string]>=[['overview','Visão geral'],['tools','Tools'],['capabilities','Capabilities'],['runtimes','Runtimes'],['roles','Roles'],['relations','Relações'],['provenance','Proveniência']];
...
export function McpAtlasApp({themeOverride,embedded=false}:{themeOverride?:string;onThemeToggle?:()=>void;embedded?:boolean}={}){
 ...
 const [theme,setTheme]=useState<McpTheme>(initialTheme),[graphView]=useState<GraphView>(initialGraphView);
 ...
 <main className="system-native" data-system-tab={tab} data-system-ready={topology?'true':'false'}>
  ...
  {topology&&tab==='overview'&&<><div className="system-stat-row">{[[topology.stats.tools,'tools',`${topology.stats.remote_tools} remotas · ${topology.stats.internal_tools} internas`],[topology.stats.capabilities,'capabilities','registradas na projeção'],[topology.stats.backends,'runtimes',topology.stats.backend_counts.unknown?`${topology.stats.backend_counts.unknown} desconhecidos`:'identificados'],[topology.stats.roles,'papéis','declarados'],[topology.links.length,'relações','publicadas']].map(([v,n,d])=><div key={String(n)}><strong>{v}</strong><span>{n}</span><small>{d}</small></div>)}</div>
   ...
   <div className="system-shortcuts">{SYSTEM_TABS.filter(([id])=>id!=='overview').map(([id,n])=>{const c=id==='tools'?topology.stats.tools:id==='capabilities'?topology.stats.capabilities:id==='runtimes'?topology.stats.backends:id==='roles'?topology.stats.roles:id==='relations'?topology.links.length:undefined;return <button key={id} type="button" onClick={()=>selectTab(id)}><span>{n}</span><strong>{c??'↗'}</strong></button>;})}</div>
  ...
  {selectedNode&&<aside className="system-row-inspector"><button type="button" onClick={()=>setSelected(null)} aria-label="Fechar detalhes">Fechar</button><strong>{selectedNode.label}</strong><span>{status(selectedNode.status)} · {kindLabel(selectedNode.kind)}</span>{selectedNode.summary&&<p>{selectedNode.summary}</p>}<a href={`#/atlas?lente=sistema&sel=${encodeURIComponent(selectedNode.id)}&view=2d`}>Ver no Mapa ↗</a></aside>}
 </main></div>;
}
