...
const isStaticWorldEndpoint=(endpoint:string)=>(endpoint.split('?',1)[0]??endpoint).endsWith('.ndjson');

export function useWorld(){
  const [world,setWorld]=useState<WorldState|null>(null),[loading,setLoading]=useState(false),[error,setError]=useState('');
  const previous=useRef<WorldState|null>(null),controller=useRef<AbortController|null>(null),lastReadAt=useRef(0);
  const refresh=useCallback(async(reset=false)=>{
    ...
    try{
      ...
      const staticProjection=isStaticWorldEndpoint(endpoint);
      const response=await fetch(endpoint,{
        signal:ctrl.signal,
        credentials:requestCredentials(endpoint),
        // Pages projections use conditional revalidation; dynamic streams remain uncached.
        cache:staticProjection?(reset?'reload':'no-cache'):'no-store',
      });
      ...
      if(last){const next=last as WorldState;next.diff=diffWorld(previous.current,next);previous.current=next;lastReadAt.current=Date.now();setWorld(next);}else throw new Error('Nenhuma leitura recebida.');
