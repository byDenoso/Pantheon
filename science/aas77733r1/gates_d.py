...
def g15(ctx:Context,results:dict)->dict:
    ...
    return gate_result("G15","PASS" if adequate else "FAIL",metrics={"baseline_zmin":settings.PRIMARY_ZMIN,"null_q95":q95,"power_grid":rows,"canonical_observed_injection":observed,"adequate_power_at_observed_level":adequate,"threshold":threshold,"historical_manuscript_target_used":False,"mocks":n})
...
def g16(ctx:Context,results:dict)->dict:
    ...
    return gate_result("G16","PASS" if robust else "FAIL",metrics={"baseline_zmin":settings.PRIMARY_ZMIN,"T_obs":obs,"curve":rows,"minimum_offset_reproducing_Tobs_at_50pct":first,"robust_to_modest_survey_offsets":robust,"mocks":n})
...
def g17(ctx:Context,results:dict)->dict:
    ...
    precision=b.precision[np.ix_(idx,idx)]; metric=GLSMetric(idx.size,precision=precision); fit=fit_omega_m(zhd=z,zhel=zhel,observed=obs,metric=metric,grid=ctx.omega_grid); residual=np.asarray(fit["residual"],float)-float(fit["best"]["intercept"]); piv=blind_pivots(z,**settings.BLIND_SCAN); scan=hard_step_scan(z=z,residual=residual,metric=metric,pivots=piv)
    ...
    pz=float(_state(ctx)["scan"]["best_z"]); shift=abs(float(scan["best_z"])-pz); consistent=p<settings.THRESHOLDS["des_replication_global_alpha"] and shift<=settings.THRESHOLDS["des_replication_max_pivot_shift"]
...
def _alt_scan(ctx:Context,z_alt:np.ndarray,keep:np.ndarray|None=None)->dict:
    ...
    z=np.asarray(z_alt,float)[idx]; zhel=s["zhel"][idx]; obs=s["observed"][idx]; fit=fit_omega_m(zhd=z,zhel=zhel,observed=obs,metric=metric,grid=ctx.omega_grid); residual=np.asarray(fit["residual"],float)-float(fit["best"]["intercept"]); piv=blind_pivots(z,**settings.BLIND_SCAN); scan=hard_step_scan(z=z,residual=residual,metric=metric,pivots=piv)
...
def g18(ctx:Context,results:dict)->dict:
    ...
    zcmb=_alt_scan(ctx,s["zcmb"]); sweeps=[]
    for threshold in [100.0,150.0,200.0,250.0,300.0,400.0,500.0]:
        keep=s["vpecerr"]<=threshold
        if np.count_nonzero(keep)<300:
            sweeps.append({"threshold_km_s":threshold,"n":int(np.count_nonzero(keep)),"status":"INSUFFICIENT_SUPPORT"}); continue
        try:
            row=_alt_scan(ctx,s["z"],keep=keep); sweeps.append({"threshold_km_s":threshold,"status":"OK",**row})
        except ValueError:
            sweeps.append({"threshold_km_s":threshold,"n":int(np.count_nonzero(keep)),"status":"INSUFFICIENT_SCAN_SUPPORT"})
    metrics={"baseline":baseline,"zHD_vs_zCMB":{"zCMB_branch":zcmb},"vpecerr_sweep":sweeps,"local_structure_external":{"status":"OPEN","fresh_crossmatch":False},"void_external":{"status":"OPEN","fresh_crossmatch":False},"physical_context_complete":False}
